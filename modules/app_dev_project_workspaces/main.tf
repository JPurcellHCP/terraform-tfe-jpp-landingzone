...
resource "tfe_workspace" "app_dev_workspaces" {
...
resource "tfe_workspace_policy_set" "test" {
  for_each = tfe_workspace.app_dev_workspaces
  policy_set_id = data.tfe_policy_set.test.id
  workspace_id  = tfe_workspace.app_dev_workspaces[each.key].id
}
...
locals {
  ...
  directory_env_mapping = { for pair in setproduct(local.directory_names, local.environments) : "${pair[1]}_${pair[0]}" => "${pair[1]}" }
}

output "project_name" {
  value = tfe_project.projects_demo.name
}